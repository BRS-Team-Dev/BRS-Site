...
document.addEventListener('DOMContentLoaded', function() {
  ...
  initStickyHeader();
  initHorizontalCascades();
  initCascadeTitleFade();
  initAnnouncementBar();
  initScrollReveal();
  initAccordions();
  initTabs();
...
/**
 * Mobile "Services" dropdown expand/collapse
 */
function initServicesDropdown() {
  const toggle = document.querySelector('.nav-mobile-services-toggle');
...
function initStickyHeader() {
  ...
  const body = document.body;
  const stacks = document.querySelectorAll('.scroll-stack');
  const firstStack = stacks[0] || null;
  const lastStack = stacks[stacks.length - 1] || null;
  let ticking = false;

  function updateHeader() {
    const y = window.scrollY;

    // Background: translucent rgba(0,0,0,0.4) at the absolute top, solid #000 once scrolled.
    if (y > 4) {
    ...
    // Disable the sticky top chrome while inside the cascade run — from when the
    // first cascade reaches the top until the last cascade has scrolled past.
    if (firstStack && lastStack) {
      const start = firstStack.getBoundingClientRect().top + y;
      const end = lastStack.getBoundingClientRect().bottom + y;
      body.classList.toggle('nav-unstuck', y >= start - 1 && y < end);
    }
...
/* ============================================
   3c. HORIZONTAL (SIDEWAYS) CASCADE
   Maps vertical scroll progress through a tall .cascade-h section
   onto a horizontal translate of its .cascade-h-track.
   data-direction="ltr" (default) or "rtl" flips the travel.
   ============================================ */

function initHorizontalCascades() {
  const sections = document.querySelectorAll('.cascade-h');
  ...
  function update() {
    ...
    sections.forEach(function(section) {
      const track = section.querySelector('.cascade-h-track');
      if (!track) return;

      // On mobile the section is a normal vertical cascade — no transform.
      if (isMobile) {
        track.style.transform = '';
        return;
      }

      const rtl = section.getAttribute('data-direction') === 'rtl';
      const scrollDist = section.offsetHeight - window.innerHeight;
      if (scrollDist <= 0) return;

      // 0 → 1 as the section scrolls from entering to leaving the pin.
      let p = -section.getBoundingClientRect().top / scrollDist;
      p = Math.max(0, Math.min(1, p));

      const maxX = track.scrollWidth - window.innerWidth;
      // LTR: 0 → -maxX (panels enter from the right).
      // RTL: -maxX → 0 (panels enter from the left; track is row-reversed).
      const x = rtl ? (p * maxX) - maxX : -(p * maxX);
      track.style.transform = 'translateX(' + x + 'px)';
    });
    ticking = false;
  }

  // Snapping: while actively scrolling the track scrubs freely (see update());
  // once scrolling stops we always settle on the NEAREST full slide so the
  // cascade never rests half-way between two panels.
  let snapTimer = null;
  ...
  function snapToEdge() {
    ...
    sections.forEach(function(section) {
      const track = section.querySelector('.cascade-h-track');
      if (!track) return;
      const panels = track.querySelectorAll('.cascade-h-panel').length;
      if (panels < 2) return;
      const scrollDist = section.offsetHeight - window.innerHeight;
      if (scrollDist <= 0) return;
      const vh = window.innerHeight;
      const startY = window.scrollY + section.getBoundingClientRect().top;
      const endY = startY + scrollDist;
      const cur = window.scrollY;

      // Vertical ENTRY — scrolling up into the first slide. Commit to the first
      // slide once it is at least half in view, otherwise settle back out.
      if (cur < startY) {
        const a = startY - vh;               // first slide just entering from the bottom
        if (cur <= a) return;                // not in the entry zone yet
        const pv = (cur - a) / vh;           // 0 -> 1 as the first slide rises to full
        const t = pv >= 0.5 ? startY : a;
        if (Math.abs(t - cur) > 2) window.scrollTo({ top: t, behavior: 'smooth' });
        return;
      }

      // Vertical EXIT — the last slide scrolling up out to the next section.
      if (cur > endY) {
        const b = endY + vh;                 // last slide fully gone
        if (cur >= b) return;                // past the exit zone
        const pv = (cur - endY) / vh;        // 0 -> 1 as the last slide leaves
        const t = pv >= 0.5 ? b : endY;
        if (Math.abs(t - cur) > 2) window.scrollTo({ top: t, behavior: 'smooth' });
        return;
      }

      // Pinned middle — snap to the nearest full slide.
      const seg = scrollDist / (panels - 1);
      let idx = Math.round((cur - startY) / seg);
      idx = Math.max(0, Math.min(panels - 1, idx));

      const target = Math.round(startY + idx * seg);
  ...
  window.addEventListener('scroll', function() {
    ...
    snapTimer = setTimeout(snapToEdge, 150);
  }, { passive: true });
...
/* ============================================
   3d. CASCADE TITLE FADE
   A sticky cascade title slide fades to invisible as the next slide
   scrolls up and covers it (so it doesn't bleed through a see-through panel).
   ============================================ */

function initCascadeTitleFade() {
  const titles = document.querySelectorAll('.cascade-title-fade');
  if (!titles.length) return;

  let ticking = false;

  function update() {
    // On tablet/mobile the cascades un-pin, so the fade is disabled —
    // keep the title fully visible.
    if (window.innerWidth <= 1024) {
      titles.forEach(function(title) { title.style.opacity = ''; title.style.pointerEvents = ''; });
      ticking = false;
      return;
    }
    const vh = window.innerHeight;
    titles.forEach(function(title) {
      const cover = title.nextElementSibling;
      if (!cover) return;
      // Opacity 1 while the covering slide is a full screen below; fades to 0
      // as it scrolls up to the top and covers the title.
      const top = cover.getBoundingClientRect().top;
      let opacity = top / vh;
      opacity = Math.max(0, Math.min(1, opacity));
      title.style.opacity = opacity;
      title.style.pointerEvents = opacity < 0.05 ? 'none' : '';
    });
    ticking = false;
  }
